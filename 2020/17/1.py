...
def printgrid(grid, dims):
    for i in range(dims[0][0], dims[0][1]+1):
        out = 'z=%d' % i
        for j in range(dims[1][0], dims[1][1]+1):
            out += '\n'+str(j) 
            for k in range(dims[2][0], dims[2][1]+1):
...
def cycle(grid, dims):
    new_grid = {}
    dim_ranges = [
        range(dims[0][0]-1, dims[0][1]+2),
        range(dims[1][0]-1, dims[1][1]+2),
        range(dims[2][0]-1, dims[2][1]+2)]
    for i in dim_ranges[0]:
        for j in dim_ranges[1]:
            for k in dim_ranges[2]:
                key = '%d,%d,%d' % (i,j,k)
                neighbors = [
                    '%d,%d,%d' % (x,y,z) 
...
for i in range(0, len(input)):
    for j in range(0, len(input[i])):
        grid.setdefault('0,%d,%d' % (i, j), input[i][j])
